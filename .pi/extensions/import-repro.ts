/**
 * Import a ziki session shared as a gist by the issue-analysis CI workflow
 ...
 *   /ir https://github.com/earendil-works/ziki/issues/123
 *
 *   ziki "/ir <gist-id>"
 */
...
const SHARE_URL_RE = /^https:\/\/ziki\.dev\/session\/#([0-9a-fA-F]{20,})(?:[/#?].*)?$/;
...
function decodeExportedHtml(html: string): { header: SessionHeader; jsonl: string } {
	...
	if (!match) throw new Error("HTML does not contain embedded ziki session data");

	let data: unknown;
	try {
		data = JSON.parse(Buffer.from(match[1], "base64").toString("utf8"));
	} catch {
		throw new Error("embedded ziki session data is not valid JSON");
	}
	...
	if (!header || header.type !== "session" || typeof header.id !== "string" || typeof header.cwd !== "string") {
		throw new Error("embedded ziki session data has no valid session header");
	}
	if (!Array.isArray(sessionData.entries)) {
		throw new Error("embedded ziki session data has no entries array");
	}
...
function getCiWorkdirName(sourceCwd: string): string | undefined {
	...
	return /^ziki-ci-[0-9a-f]{32}$/i.test(name) ? name : undefined;
}
