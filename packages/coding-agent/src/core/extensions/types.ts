...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@iamjoyeb/ziki-coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@iamjoyeb/ziki-coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown ziki and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Ziki loaded without re-discovering resources. */
...
/** Configuration for registering a provider via ziki.registerProvider(). */
...
export type ExtensionFactory = (ziki: ExtensionAPI) => void | Promise<void>;
...
/**
 * Action implementations for ziki.* API methods.
