...
function readPiManifest(packageJsonPath: string): PiManifest | null {
	try {
		...
		if (pkg.ziki && typeof pkg.ziki === "object") {
			return pkg.ziki as PiManifest;
		}
		return null;
	} catch {
		return null;
 ...
 *
 * Checks for:
 * 1. package.json with "ziki.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "ziki" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "ziki" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with ziki manifest or index.ts
