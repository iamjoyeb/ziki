...
import { type ImageContent, modelsAreEqual } from "@zikilabs/ziki-ai";
...
const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "ziki -ne".';
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.ZIKI_OFFLINE);
	if (offlineMode) {
	...
	if (await handlePackageCommand(args, { extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `ziki update` drain naturally instead.
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.ZIKI_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: ZIKI_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
