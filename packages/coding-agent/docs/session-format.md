...
```
~/.ziki/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.ziki/agent/sessions/`.

Ziki also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, ziki uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/@zikilabs/ziki-coding-agent/dist/` and `node_modules/@zikilabs/ziki-ai/dist/`.
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if ziki-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `ziki.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `ziki.setSessionName()` in extensions.
