> ziki can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to ziki's agent capabilities. Use it to embed ziki in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @zikilabs/ziki-coding-agent
```
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `ziki.sendMessage()`.
...
The `Agent` class (from `@zikilabs/ziki-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.ziki/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.ziki/extensions/`)
- Project skills:
  - `.ziki/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.ziki/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.ziki/agent/skills/`)
  - `~/.agents/skills/`
...
import { ModelRuntime } from "@zikilabs/ziki-coding-agent";
...
```typescript
import { InMemoryCredentialStore } from "@zikilabs/ziki-ai";
import { createAgentSession, ModelRuntime } from "@zikilabs/ziki-coding-agent";

// Default: uses ~/.ziki/agent/auth.json and ~/.ziki/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@zikilabs/ziki-coding-agent";
...
The `edit` tool returns `details.diff` for Ziki's TUI display and `details.patch` as a standard unified patch for SDK consumers.
...
import { createAgentSession, defineTool } from "@zikilabs/ziki-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `ziki.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `ziki.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.ziki/agent/extensions/`, `.ziki/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@zikilabs/ziki-coding-agent";
...
**Event Bus:** Extensions can communicate via `ziki.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@zikilabs/ziki-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@zikilabs/ziki-coding-agent";
...
1. Global: `~/.ziki/agent/settings.json`
2. Project: `<cwd>/.ziki/settings.json`
