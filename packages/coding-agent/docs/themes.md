> ziki can create themes. Ask it to build one for your setup.
...
Ziki loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.ziki/agent/themes/*.json`
- Project: `.ziki/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `ziki.themes` entries in `package.json`
...
On first run, ziki detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.ziki/agent/themes
vim ~/.ziki/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/earendil-works/ziki/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, ziki reloads it automatically for immediate visual feedback.
...
Ziki uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, ziki falls back to the nearest approximation.
