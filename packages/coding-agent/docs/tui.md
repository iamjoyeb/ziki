> ziki can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@zikilabs/ziki-tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@zikilabs/ziki-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `ZIKI_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@zikilabs/ziki-tui";
...
Import from `@zikilabs/ziki-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@zikilabs/ziki-tui";
```
...
```typescript
import { matchesKey, Key } from "@zikilabs/ziki-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@zikilabs/ziki-tui";
...
```typescript
import { getMarkdownTheme } from "@zikilabs/ziki-coding-agent";
import { Markdown } from "@zikilabs/ziki-tui";
...
Set `ZIKI_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
ZIKI_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@zikilabs/ziki-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@zikilabs/ziki-coding-agent";
import { DynamicBorder } from "@zikilabs/ziki-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@zikilabs/ziki-tui";
...
```typescript
import { BorderedLoader } from "@zikilabs/ziki-coding-agent";

ziki.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@zikilabs/ziki-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@zikilabs/ziki-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@zikilabs/ziki-tui";

ziki.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while ziki is streaming a response.
...
// Restore ziki's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@zikilabs/ziki-tui";
