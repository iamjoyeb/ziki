> ziki can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend ziki's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.ziki/agent/extensions/` (global) or `.ziki/extensions/` (project-local) for auto-discovery. Use `ziki -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `ziki.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `ziki.registerCommand()`
- **Session persistence** - Store state that survives restarts via `ziki.appendEntry()`
...
Create `~/.ziki/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.ziki/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.ziki/agent/extensions/*.ts` | Global (all projects) |
| `~/.ziki/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.ziki/extensions/*.ts` | Project-local |
| `.ziki/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as ziki packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@zikilabs/ziki-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@zikilabs/ziki-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@zikilabs/ziki-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed ziki packages installed with `ziki install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (ziki: ExtensionAPI) {
  // Subscribe to events
  ziki.on("event_name", async (event, ctx) => {
  ...
  ziki.registerTool({ ... });
  ziki.registerCommand("name", { ... });
  ziki.registerShortcut("ctrl+x", { ... });
  ziki.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, ziki awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `ziki.registerProvider()` are flushed.
...
This pattern makes the fetched models available during normal startup and to `ziki --list-models`.
...
thinking level changes (settings, keybinding, ziki.setThinkingLevel())
...
Fired before ziki decides whether to trust a project with dynamic configs (`.ziki` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
ziki.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether ziki asks, trusts, or declines by default.
...
```typescript
ziki.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
Fired when the current session display name is set via `/name`, RPC, or `ziki.setSessionName()`.

```typescript
ziki.on("session_info_changed", async (event, ctx) => {
...
```typescript
ziki.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, ziki emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
ziki.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, ziki emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
ziki.on("session_compact", async (event, ctx) => {
...
```typescript
ziki.on("session_before_tree", async (event, ctx) => {
...
ziki.on("session_tree", async (event, ctx) => {
...
```typescript
ziki.on("session_shutdown", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Ziki uses to build the system prompt. This lets you inspect what Ziki has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later `before_agent_start` handlers can still modify it again.

#### agent_start / agent_end / agent_settled

`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but Ziki may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know Ziki will not continue running automatically.

```typescript
ziki.on("agent_start", async (_event, ctx) => {});

ziki.on("agent_end", async (event, ctx) => {
  // event.messages - messages from this low-level run
});

ziki.on("agent_settled", async (_event, ctx) => {
...
```typescript
ziki.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

ziki.on("turn_end", async (event, ctx) => {
...
```typescript
ziki.on("message_start", async (event, ctx) => {
  // event.message
});

ziki.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

ziki.on("message_end", async (event, ctx) => {
...
```typescript
ziki.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

ziki.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

ziki.on("tool_execution_end", async (event, ctx) => {
...
```typescript
ziki.on("context", async (event, ctx) => {
...
```typescript
ziki.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header ziki adds for this call
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Ziki's system prompt string rather than the final serialized provider payload.

```typescript
ziki.on("before_provider_request", (event, ctx) => {
...
```typescript
ziki.on("after_provider_response", (event, ctx) => {
...
```typescript
ziki.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `ziki.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, ziki waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@zikilabs/ziki-coding-agent";
...
```typescript
import { isBashToolResult } from "@zikilabs/ziki-coding-agent";

ziki.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@zikilabs/ziki-coding-agent";

ziki.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap ziki's built-in local bash backend
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.ziki` when constructing project-local config paths. Rebranded distributions can use a different config directory name.

```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@zikilabs/ziki-coding-agent";
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while ziki is idle.
...
Control flow helpers. `ctx.isIdle()` is false while Ziki is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of ziki.
...
Returns Ziki's current system prompt string.
...
```typescript
ziki.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Ziki currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `ziki` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@zikilabs/ziki-coding-agent";

ziki.registerCommand("switch", {
...
- Captured old `ziki` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
ziki.registerCommand("reload-runtime", {
...
export default function (ziki: ExtensionAPI) {
  ...
  ziki.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      ziki.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### ziki.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### ziki.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`ziki.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `ziki.getAllTools()` and are callable by the LLM without `/reload`.

Use `ziki.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
### ziki.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`ziki.appendEntry()`](#piappendentrycustomtype-data) with [`ziki.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### ziki.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
ziki.sendUserMessage("What is 2+2?");

// With content array (text + images)
ziki.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
ziki.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
ziki.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### ziki.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `ziki.registerEntryRenderer()`.

```typescript
ziki.appendEntry("my-state", { count: 42 });
ziki.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### ziki.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
ziki.setSessionName("Refactor auth module");
```
...
```typescript
const name = ziki.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### ziki.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
ziki.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
ziki.setLabel(entryId, undefined);
...
### ziki.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, ziki keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.

```typescript
ziki.registerCommand("stats", {
...
```typescript
import type { AutocompleteItem } from "@zikilabs/ziki-tui";

ziki.registerCommand("deploy", {
...
```typescript
const commands = ziki.getCommands();
...
### ziki.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `ziki.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### ziki.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `ziki.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@zikilabs/ziki-tui";

ziki.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### ziki.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
ziki.registerShortcut("ctrl+shift+p", {
...
### ziki.registerFlag(name, options)

Register a CLI flag.

```typescript
ziki.registerFlag("plan", {
...
// Check value
if (ziki.getFlag("plan")) {
  // Plan mode enabled
}
```

### ziki.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await ziki.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### ziki.getActiveTools() / ziki.getAllTools() / ziki.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `ziki.getActiveTools()` returns the active tool names as `string[]`; `ziki.getAllTools()` returns metadata for all configured tools.

```typescript
const active = ziki.getActiveTools(); // ["read", "bash", ...]
const all = ziki.getAllTools();
// all = [{
//   name: "read",
...
ziki.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
ziki.setActiveTools(["read", "bash"]); // Switch to read-only
```

`ziki.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await ziki.setModel(model);
  if (!success) {
...
### ziki.getThinkingLevel() / ziki.setThinkingLevel(level)
...
```typescript
const current = ziki.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
ziki.setThinkingLevel("high");
```
...
```typescript
ziki.events.on("my:event", (data) => { ... });
ziki.events.emit("my:event", { ... });
```

### ziki.registerProvider(name, config)

Register or override a model provider dynamically. Useful for proxies, custom endpoints, or team-wide model configurations.

Calls made during the extension factory function are queued and applied once the runner initialises. Calls made after that — for example from a command handler following a user setup flow — take effect immediately without requiring a `/reload`.

Dynamic providers can implement `refreshModels`. Ziki calls it during model refresh, publishes the returned list synchronously through the provider, and passes the canonical credential/store/network/signal context. The extension decides whether to persist the catalog through `context.store`; live servers such as llama.cpp can ignore it.

Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@zikilabs/ziki-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.
...
ziki.registerProvider(provider);

// Register a new provider with custom models
ziki.registerProvider("my-proxy", {
  name: "My Proxy",
...
ziki.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
```typescript
ziki.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    ziki.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `ziki.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `ziki.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@zikilabs/ziki-coding-agent";
...
ziki.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via ziki.exec (captured from extension closure)
    const result = await ziki.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Ziki persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@zikilabs/ziki-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when ziki resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
ziki --no-builtin-tools -e ./my-extension.ts
```
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@zikilabs/ziki-coding-agent";
...
For `user_bash`, extensions can reuse ziki's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
`createBashTool()` exposes the current session to commands through `ZIKI_SESSION_ID`, `ZIKI_SESSION_FILE`, `ZIKI_PROVIDER`, `ZIKI_MODEL`, and `ZIKI_REASONING_LEVEL`. Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
export default function (ziki: ExtensionAPI) {
  ...
  ziki.registerTool({ name: "db_connect", ... });
  ziki.registerTool({ name: "db_query", ... });
  ziki.registerTool({ name: "db_close", ... });

  ziki.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
```typescript
import { keyHint } from "@zikilabs/ziki-coding-agent";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `ziki.setActiveTools()` during execution. Ziki detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `ziki.registerTool()` so it appears in `ziki.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `ziki.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. Ziki records which tools were added on the loader's tool result.
5. Before the next model response, Ziki exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `ziki.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Ziki adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Ziki sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Ziki also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (ziki: ExtensionAPI) {
  ...
  ziki.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = ziki.getAllTools()
      ...
      const active = ziki.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      ziki.setActiveTools([...new Set([...active, ...added])]);
  ...
  ziki.on("session_start", () => {
    ...
    const initialTools = ziki.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    ziki.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("ziki - my-project");
...
```typescript
import { Text, Component } from "@zikilabs/ziki-tui";
...
import { matchesKey } from "@zikilabs/ziki-tui";
...
ziki.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `ziki.sendMessage()`:
...
```typescript
ziki.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

ziki.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@zikilabs/ziki-coding-agent";
...
| `event-bus.ts` | Inter-extension events | `ziki.events` |
| **Session Metadata** |||
