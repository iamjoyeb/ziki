...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".ziki", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".ziki", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".ziki", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".ziki"), { recursive: true });
			writeFileSync(join(cwd, ".ziki", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".ziki"), { recursive: true, force: true });
