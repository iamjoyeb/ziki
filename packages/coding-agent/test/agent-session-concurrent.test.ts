...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `ziki-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(ziki) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = ziki;
			},
			(ziki) => {
		...
		expect(ziki).toBeDefined();

		ziki!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
