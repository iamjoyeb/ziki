...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.ZIKI_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.ZIKI_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .ziki skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .ziki", async () => {
			const extDir = join(tempDir, ".ziki", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".ziki"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".ziki", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".ziki", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".ziki", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".ziki", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".ziki", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json ziki.extensions in extensions setting", async () => {
			// Create a package with ziki.extensions in package.json
			...
			// Should find the extensions declared in package.json ziki.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .ziki/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-ziki", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-ziki"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-ziki\ndescription: user ziki\n---\n");
		...
		it("should use the project .ziki dir as baseDir for project .ziki skills", async () => {
			const projectBaseDir = join(tempDir, ".ziki");
			const skillPath = join(projectBaseDir, "skills", "project-ziki", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-ziki"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-ziki\ndescription: project ziki\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".ziki", "agent");
		...
		it("should dedupe user skill entries when ~/.ziki/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .ziki auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".ziki\n");

			const skillDir = join(tempDir, ".ziki", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with ziki manifest", async () => {
		...
		it("should keep ziki manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.ziki\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".ziki", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the ziki-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "ziki-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .ziki settings base", () => {
			...
			const rel = relative(join(tempDir, ".ziki"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in ziki manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "ziki-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "ziki-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:ziki-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:ziki-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".ziki"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json ziki.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".ziki", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".ziki", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".ziki", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".ziki", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".ziki", "npm", "node_modules", "example");
