...
import { Agent, type AgentEvent, type AgentTool } from "@zikilabs/ziki-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@zikilabs/ziki-ai/compat";
...
describe("AgentSession retry", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `ziki-retry-test-${Date.now()}`);
