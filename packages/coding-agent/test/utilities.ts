...
import type { OAuthCredentials } from "@zikilabs/ziki-ai";
import { getModel, streamSimple } from "@zikilabs/ziki-ai/compat";
import { builtinProviders } from "@zikilabs/ziki-ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.ziki/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".ziki", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.ziki/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.ziki/agent/auth.json
 */
...
/** Path to the real ziki agent config directory */
export const ZIKI_AGENT_DIR = join(homedir(), ".ziki", "agent");

/**
 * Get an AuthStorage instance backed by ~/.ziki/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `ziki-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
