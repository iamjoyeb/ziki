...
describe("extension active tools next-turn refresh", () => {
	it("applies ziki.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(ziki) => {
				ziki.registerTool({
					...
					execute: async () => {
						ziki.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(ziki) => {
				ziki.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						ziki.setActiveTools([...ziki.getActiveTools(), "after_load"]);
						return {
