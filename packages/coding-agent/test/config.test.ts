...
const originalPiPackageDir = process.env.ZIKI_PACKAGE_DIR;
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "ziki-pnpm-"));
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "ziki-yarn-"));
	const binDir = join(temp, "bin");
...
function createBunGlobalInstall(): { packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "ziki-bun-"));
...
describe("detectInstallMethod", () => {
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("@mariozechner/pi-coding-agent", undefined, "@new-scope/ziki");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/ziki"],
			display: `npm --prefix ${prefix} uninstall -g @mariozechner/pi-coding-agent && npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/ziki`,
			steps: [
				{
					command: "npm",
				...
				},
				{
					...
					display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/ziki`,
				},
			],
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "pnpm",
			args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/ziki"],
			display:
				"pnpm remove -g @mariozechner/pi-coding-agent && pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/ziki",
			steps: [
				{
					command: "pnpm",
				...
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/ziki"],
					display: "pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/ziki",
				},
			],
		});
	});
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "yarn",
			args: ["global", "add", "--ignore-scripts", "@new-scope/ziki"],
			display:
				"yarn global remove @mariozechner/pi-coding-agent && yarn global add --ignore-scripts @new-scope/ziki",
			steps: [
				{
					command: "yarn",
				...
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/ziki"],
					display: "yarn global add --ignore-scripts @new-scope/ziki",
				},
			],
		});
	});
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/ziki"],
			display:
				"bun uninstall -g @mariozechner/pi-coding-agent && bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/ziki",
			steps: [
				{
					command: "bun",
				...
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/ziki"],
					display: "bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/ziki",
				},
			],
		});
	});
