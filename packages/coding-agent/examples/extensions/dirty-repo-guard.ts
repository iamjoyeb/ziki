...
async function checkDirtyRepo(
	...
	const { stdout, code } = await ziki.exec("git", ["status", "--porcelain"]);
...
export default function (ziki: ExtensionAPI) {
	ziki.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(ziki, ctx, action);
	});

	ziki.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(ziki, ctx, "fork");
	});
}
