 ...
 * - ~/.ziki/agent/presets.json (global)
 * - <cwd>/.ziki/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `ziki --preset plan` - start with plan preset
...
export default function presetExtension(ziki: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: ziki.getThinkingLevel(),
				tools: ziki.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			ziki.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = ziki.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				ziki.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await ziki.setModel(originalState.model);
				}
				ziki.setThinkingLevel(originalState.thinkingLevel);
				ziki.setActiveTools(originalState.tools);
			} else {
				ziki.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	ziki.registerShortcut(Key.ctrlShift("u"), {
	...
	ziki.on("before_agent_start", async (event) => {
	...
	ziki.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = ziki.getFlag("preset");
	...
	ziki.on("turn_start", async () => {
		if (activePresetName) {
			ziki.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
