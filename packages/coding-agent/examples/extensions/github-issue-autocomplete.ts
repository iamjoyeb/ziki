...
async function resolveGitHubRepo(ziki: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await ziki.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (ziki: ExtensionAPI): void {
	ziki.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(ziki, ctx.cwd);
		if (!resolvedRepo.ok) {
