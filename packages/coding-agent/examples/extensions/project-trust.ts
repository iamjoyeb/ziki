 ...
 *
 *   mkdir -p ~/.ziki/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.ziki/agent/extensions/
 *
 * Or:
 *
 *   ziki -e packages/coding-agent/examples/extensions/project-trust.ts
 *
...
export default function (ziki: ExtensionAPI) {
	...
	ziki.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
