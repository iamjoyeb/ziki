 ...
 *
 * Usage: ziki --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@zikilabs/ziki-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@zikilabs/ziki-tui";
...
export default function (ziki: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like ziki-doom would need)
	ziki.registerCommand("overlay-animation", {
	...
	ziki.registerCommand("overlay-anchors", {
	...
	ziki.registerCommand("overlay-margins", {
	...
	ziki.registerCommand("overlay-stack", {
	...
	ziki.registerCommand("overlay-overflow", {
	...
	ziki.registerCommand("overlay-edge", {
	...
	ziki.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	ziki.registerCommand("overlay-maxheight", {
	...
	ziki.registerCommand("overlay-sidepanel", {
	...
	ziki.registerCommand("overlay-toggle", {
	...
	ziki.registerCommand("overlay-passive", {
	...
	ziki.registerCommand("overlay-focus", {
	...
	ziki.registerCommand("overlay-streaming", {
...
// Animation demo - proves overlays can handle real-time updates like ziki-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (ziki-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
