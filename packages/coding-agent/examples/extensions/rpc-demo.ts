...
export default function (ziki: ExtensionAPI) {
	...
	ziki.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "ziki RPC Demo (new session)" : "ziki RPC Demo");
	...
	ziki.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	ziki.on("turn_end", async (_event, ctx) => {
	...
	ziki.registerCommand("rpc-input", {
	...
	ziki.registerCommand("rpc-editor", {
	...
	ziki.registerCommand("rpc-prefill", {
