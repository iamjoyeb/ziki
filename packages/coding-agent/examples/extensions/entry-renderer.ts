 ...
 * to the LLM. Custom entries are stored in the session via ziki.appendEntry() and
 * rendered in interactive mode via ziki.registerEntryRenderer().
 *
...
export default function (ziki: ExtensionAPI) {
	ziki.registerEntryRenderer<StatusCardData>("status-card", (entry, { expanded }, theme) => {
	...
	ziki.registerCommand("status-card", {
		description: "Render a durable status card that is not sent to the LLM",
		handler: async (args) => {
			ziki.appendEntry<StatusCardData>("status-card", {
