 ...
 *
 * Runs ziki's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   ziki -e /path/to/ziki/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (ziki: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `ziki ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	ziki.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
