...
export default function (ziki: ExtensionAPI) {
	ziki.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await ziki.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[ziki] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await ziki.exec("git", ["add", "-A"]);
		const { code: commitCode } = await ziki.exec("git", ["commit", "-m", commitMessage]);
