...
export default function (ziki: ExtensionAPI) {
	// Register a /quit command that cleanly exits ziki
	ziki.registerCommand("quit", {
		description: "Exit ziki cleanly",
	...
	ziki.registerTool({
		...
		description: "Complete a task and exit ziki",
	...
	ziki.registerTool({
		...
		description: "Deploy the application and exit ziki",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await ziki.exec("npm", ["run", "deploy", params.environment], { signal });
