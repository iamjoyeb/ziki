 ...
 *   ziki -e ./examples/extensions/git-merge-and-resolve.ts
 */
...
async function findConflicts(ziki: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await ziki.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (ziki: ExtensionAPI) {
	ziki.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await ziki.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await ziki.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await ziki.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await ziki.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await ziki.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await ziki.exec("git", ["merge", "--no-ff", ref]);
			if (mergeCode === 0) return;
		}

		// Either we just merged with conflicts, or we were already in an unfinished merge
		const conflicts = await findConflicts(ziki, ctx.cwd);
		if (conflicts.length === 0) return;

		ziki.sendUserMessage(formatConflicts(ref, conflicts), { deliverAs: "followUp" });
	});
}
