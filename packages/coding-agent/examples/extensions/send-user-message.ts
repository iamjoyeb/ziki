 ...
 *
 * Demonstrates ziki.sendUserMessage() for sending user messages from extensions.
 * Unlike ziki.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (ziki: ExtensionAPI) {
	...
	ziki.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			ziki.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	ziki.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				ziki.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	ziki.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				ziki.sendUserMessage(args, { deliverAs: "followUp" });
