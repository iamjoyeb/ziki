 ...
 *
 * Usage:
 *   ziki -e ./ssh.ts --ssh user@host
 *   ziki -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (ziki: ExtensionAPI) {
	ziki.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	ziki.on("session_start", async (_event, ctx) => {
		...
		const arg = ziki.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
